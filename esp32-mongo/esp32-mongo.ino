...
void connectWiFiManager() {
  ...
  // WiFiManager portal CSS v2 - fixed visibility, compact layout
  const char* customCSS = "<style>"
    "* { margin:0; padding:0; box-sizing:border-box; }"
    "body { background:#111318; color:#e2e8f0; font-family:sans-serif; min-height:100vh; font-size:15px; }"
    ".wrap { max-width:400px; margin:0 auto; padding:16px 14px 36px; }"
    ".fw-header { text-align:center; padding:14px 0 10px; }"
    ".fw-logo { font-size:1.7em; font-weight:700; color:#ff6b35; }"
    ".fw-tagline { color:#64748b; font-size:0.72em; margin-top:2px; text-transform:uppercase; }"
    ".fw-badge { display:inline-block; background:rgba(255,107,53,0.15); border:1px solid rgba(255,107,53,0.35); color:#ff8c42; font-size:0.7em; font-weight:600; padding:2px 10px; border-radius:20px; margin-top:6px; }"
    ".steps { display:flex; align-items:center; gap:4px; margin:10px 0 12px; padding:10px 12px; background:#1a1f2e; border-radius:10px; border:1px solid #252d3d; }"
    ".step { flex:1; text-align:center; }"
    ".step-num { width:22px; height:22px; border-radius:50%; background:rgba(255,107,53,0.2); border:1.5px solid rgba(255,107,53,0.5); color:#ff8c42; font-size:0.72em; font-weight:700; display:flex; align-items:center; justify-content:center; margin:0 auto 3px; }"
    ".step-text { color:#94a3b8; font-size:0.62em; line-height:1.2; }"
    ".step-divider { width:1px; height:24px; background:#252d3d; flex-shrink:0; }"
    ".card, form { background:#161b27; border-radius:10px; padding:14px; margin:8px 0; border:1px solid #252d3d; }"
    "h1 { font-size:1.2em; font-weight:700; text-align:center; color:#e2e8f0; margin-bottom:4px; }"
    "h2, h3 { color:#94a3b8; text-align:center; font-weight:500; font-size:0.85em; margin-bottom:12px; }"
    ".c { text-align:center; margin-bottom:10px; }"
    "input[type='submit'], button, .btn { background:#e85d20; border:none; color:#fff; padding:11px 18px; border-radius:8px; cursor:pointer; font-weight:600; font-size:0.88em; width:100%; margin:5px 0 0; }"
    "input[type='submit']:active, button:active { opacity:0.85; }"
    ".btn-secondary { background:#1e2a3a !important; color:#94a3b8 !important; border:1px solid #2d3748 !important; }"
    "input[type='text'], input[type='password'], select { width:100%; background:#0d1117; border:1.5px solid #252d3d; color:#e2e8f0; border-radius:8px; padding:10px 12px; font-size:0.88em; margin:4px 0 10px; font-family:inherit; }"
    "input[type='text']:focus, input[type='password']:focus { outline:none; border-color:#ff6b35; }"
    "input::placeholder { color:#3d4f63; }"
    "label { display:block; color:#94a3b8; font-weight:500; margin-bottom:2px; font-size:0.82em; }"
    ".q { background:#1a1f2e; border:1px solid #252d3d; border-radius:8px; margin:5px 0; padding:10px 12px; cursor:pointer; display:flex; align-items:center; justify-content:space-between; }"
    ".q:active { border-color:#ff6b35; background:#1e2535; }"
    ".l { color:#f1f5f9 !important; font-weight:600; font-size:0.88em; flex:1; }"
    ".s { color:#e2e8f0 !important; font-weight:600; font-size:0.72em; background:#253347; padding:2px 7px; border-radius:10px; border:1px solid #2d4060; white-space:nowrap; margin-left:6px; }"
    ".msg { background:rgba(255,107,53,0.12); border:1px solid rgba(255,107,53,0.3); color:#ff8c42; padding:9px 12px; border-radius:8px; margin:8px 0; font-weight:500; text-align:center; font-size:0.83em; }"
    ".error { background:rgba(220,53,69,0.12) !important; border-color:rgba(220,53,69,0.3) !important; color:#f87171 !important; }"
    ".success { background:rgba(34,197,94,0.12) !important; border-color:rgba(34,197,94,0.3) !important; color:#4ade80 !important; }"
    ".info { color:#475569; font-size:0.76em; text-align:center; margin:5px 0; }"
    ".tip { background:#1a1f2e; border-left:3px solid #ff6b35; border-radius:0 7px 7px 0; padding:9px 11px; margin:10px 0 0; font-size:0.76em; color:#94a3b8; line-height:1.4; }"
    ".tip strong { color:#ff8c42; }"
    "input[type='checkbox'] { margin-right:6px; accent-color:#ff6b35; }"
    "hr { border:none; height:1px; background:#252d3d; margin:12px 0; }"
    "</style>"
    "<script>"
    "document.addEventListener('DOMContentLoaded',function(){"
      ...
      "}"
      /* Force text colors via inline styles — overrides WiFiManager's own CSS */
      "function fixColors(){"
        ...
        "document.querySelectorAll('.q').forEach(function(el){"
          ...
          "el.style.border='1px solid #252d3d';"
