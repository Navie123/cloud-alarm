...
function updateGasSensorUI(data) {
  ...
  // CO hint: simple plain-language status based on range
  ...
  // AQI hint: simple plain-language status based on range
  const aqiHint = document.getElementById('aqiHintText');
  if (aqiHint) {
    if (!deviceOnline || data.sensorWarmup) {
      aqiHint.textContent = 'Good below 50 AQI';
    } else {
      const aqi = Math.round(data.aqi || 0);
      if (aqi <= 50)       aqiHint.textContent = 'Good';
      else if (aqi <= 100) aqiHint.textContent = 'Moderate';
      else if (aqi <= 150) aqiHint.textContent = 'Poor';
      else                 aqiHint.textContent = 'Unhealthy — ventilate';
    }
  }
