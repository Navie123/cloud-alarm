...
// Request admin email change (send verification code)
router.post('/admin/change-email/request', verifyHouseholdSession, verifyAdminSession, async (req, res) => {
  try {
    ...
    // Check if email is already in use by another household
    const existingHousehold = await Household.findOne({ 
      'admin.email': newEmail,
      householdId: { $ne: req.household.householdId }
    });
    
    if (existingHousehold) {
      return res.status(400).json({ error: 'This email is already registered as Admin for another household' });
    }
    
    // Check if it's the same as current email
    if (req.household.admin.email === newEmail) {
      return res.status(400).json({ error: 'This is already your current email address' });
    }
    
    // Generate and send verification code
    const verificationCode = req.household.generateOTP('admin_email_change');
    req.household.pendingAdminEmail = newEmail; // Store pending email
    await req.household.save();
    
    // Send verification email
    const { sendOTPEmail } = require('../utils/email');
    await sendOTPEmail(newEmail, verificationCode, 'admin_email_change');
    
    console.log(`[Admin Email Change] Verification code sent to: ${newEmail}`);
...
router.post('/admin/change-email/verify', verifyHouseholdSession, verifyAdminSession, async (req, res) => {
  try {
    ...
    if (req.household.pendingAdminEmail !== newEmail) {
      return res.status(400).json({ error: 'Invalid email change request' });
    }
    
    // Verify OTP
    if (!req.household.verifyOTP(verificationCode, 'admin_email_change')) {
      return res.status(400).json({ error: 'Invalid or expired verification code' });
    }
    
    // Update admin email
    const oldEmail = req.household.admin.email;
    req.household.admin.email = newEmail;
    req.household.admin.emailVerified = true;
    req.household.pendingAdminEmail = undefined; // Clear pending email
    req.household.clearOTP();
    
    await req.household.save();
    
    console.log(`[Admin Email Change] Email updated from ${oldEmail} to ${newEmail} for household ${req.household.householdId}`);
    ...
    console.error('Admin email change verification error:', error);
